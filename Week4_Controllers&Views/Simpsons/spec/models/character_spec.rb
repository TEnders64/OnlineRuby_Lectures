...
RSpec.describe Character, type: :model do
    ...
    it "must provide last name" do
        @char = Character.new(first_name: 'Homer', last_name: '', workplace:'Nuclear Plant')
        expect(@char).to be_invalid()
    end
    it "must provide workplace" do
        @char = Character.new(first_name: 'Homer', last_name: 'Simpson', workplace:'')
        expect(@char).to be_invalid()
    end
    it "must be unique in table" do
        ...
        @char2 = Character.new(first_name: 'Homer', last_name: 'Simpson', workplace:'Nuclear Plant')
        expect(@char2.valid?).to eq(false)
    end
    it "must have a method called full_name that displays full name" do
        @char1 = Character.create(first_name: 'Homer', last_name: 'Simpson', workplace:'Nuclear Plant')
        expect(@char1.full_name).to eq('Homer Simpson')
    end
end
